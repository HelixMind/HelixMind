...
const request = async function<T>(endpoint: string, options: {
    ...
    try {
        ...
        if (response.ok != true) {
            throw new Error(output.error ?? await response.text() ?? "Something went wrong, try again");
        }
        ...
        throw (error instanceof Error ? error.message : "Something went wrong, try again")
    }
}
